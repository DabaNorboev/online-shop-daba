...
class UserProduct extends Model
{
    ...
    public function getAllByUserId($userId): array
    {
        ...
        foreach ($userProducts as $userProduct){
            $userProductsArray[] = new UserProductEntity($userProduct['id'], $userProduct['user_id'], $userProduct['product_id'], $userProduct['quantity']);
        }
    ...
    public function getCartProductsByUserId(string $userId): array
    {
        $stmt = $this->pdo->prepare("SELECT products.id, products.name, products.price, products.img_url, user_products.user_id, user_products.quantity FROM products JOIN user_products ON products.id = user_products.product_id WHERE user_id = :user_id");
        $stmt->execute(['user_id' => $userId]);
        ...
        foreach ($cartProducts as $cartProduct){
            $cartProductsArray[] = new CartProductEntity($cartProduct['id'], $cartProduct['name'], $cartProduct['price'], $cartProduct['img_url'], $cartProduct['quantity']);
        }
